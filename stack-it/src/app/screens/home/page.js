...
export default function HomePage() {
  ...
  return (
    <div
      className={`min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans`}
    >
        ...
        <div className="flex items-center border-b border-gray-200 dark:border-gray-700 mb-6">
          {filters.map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-3 sm:px-4 py-2 text-sm font-medium -mb-px border-b-2 ${
                filter === f
